/**
 * @package @avant-dev/akp-core — MIT sublicense
 ...
 *
 * Attribution required per AKP LICENSE Section 3:
...
export interface AKPSession {
  ...
  status: AKPSessionStatus; created_at: string; updated_at: string;
}

export interface AKPMessage {
  id: string; session_id: string; role: AKPMessageRole; content: string;
  tool_calls?: unknown | null; tool_results?: unknown | null;
...
export interface AKPAddMessageParams {
  session_id: string; role: AKPMessageRole; content: string;
...
export interface AKPIngestParams {
  content: string; collection?: string; title?: string;
  metadata?: Record<string, unknown>; tags?: string[]; org_id?: number;
}

export interface AKPIngestBatchResult { ids: number[]; count: number; }

export interface AKPSearchParams {
  query: string; collection?: string; org_id?: number; tags?: string[];
  limit?: number; threshold?: number;
}

export interface AKPRagObservation {
  id: number; query_text: string | null; collection: string;
  results_count: number; top_similarity: number | null;
  latency_ms: number; model_used: string; created_at: string;
}
...
export interface AKPChatMessage { role: 'user' | 'assistant' | 'system'; content: string; }
export interface AKPChatRequest {
  messages: AKPChatMessage[]; tier?: 'fast' | 'standard' | 'advanced';
  provider?: AKPProvider; tools?: string[]; session_id?: string;
  stream?: boolean; max_steps?: number; system?: string;
}
export interface AKPChatResponse {
  text: string; usage: { promptTokens: number; completionTokens: number };
  finish_reason?: string; tool_results?: unknown[];
}
...
export interface AKPHealthResponse {
  status: 'ok' | 'error'; server: string; version?: string;
  provider: string; embedding_provider: string; ts: string;
}
