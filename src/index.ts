...
app.use((req, res, next) => {
  ...
  if (req.headers['authorization'] !== `Bearer ${KERNEL_API_KEY}`) return res.status(401).json({ error: 'Unauthorized' });
  next();
});
...
async function boot() {
  // Load private add-on tools (gitignored — from ai-kernel-add-ons)
  try {
    ...
    if (mod?.ALL_TOOLS && typeof mod.ALL_TOOLS === 'object') {
      ...
      console.log(`[kernel] add-ons: ${getToolNames().join(', ')}`);
    }
  } catch {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[ai-kernel-protocol] :${PORT}  provider=${process.env.AI_PROVIDER ?? 'anthropic'}  tools=${getToolNames().length}`);
    if (!KERNEL_API_KEY) console.warn('[ai-kernel-protocol] WARNING: KERNEL_API_KEY not set');
  });
}
