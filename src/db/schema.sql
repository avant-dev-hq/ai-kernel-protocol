-- AI Kernel Protocol — PostgreSQL schema
...
-- ── RAG Documents (OpenAI / default — vector(1536)) ──────────────────────────
...
CREATE INDEX IF NOT EXISTS rag_documents_mistral_embedding_idx
  ON meta.rag_documents_mistral USING ivfflat (embedding vector_cosine_ops)
  WITH (lists = 100);
...
CREATE INDEX IF NOT EXISTS assistant_sessions_updated_idx ON meta.assistant_sessions (updated_at DESC);
...
CREATE INDEX IF NOT EXISTS assistant_messages_session_idx
  ON meta.assistant_messages (session_id, created_at);
